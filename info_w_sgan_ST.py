...
def generator(x, isTrain=True, reuse=False):
    with tf.variable_scope('generator', reuse=reuse):
        # 1st hidden layer
        conv1 = tf.layers.conv2d_transpose(x, filter_num[0], [4, 4], strides=(2, 2), padding='same')
        relu1 = relu(tf.layers.batch_normalization(conv1, training=isTrain))

        # 2nd hidden layer
        conv2 = tf.layers.conv2d_transpose(relu1, filter_num[1], [4, 4], strides=(2, 2), padding='same')
        relu2 = relu(tf.layers.batch_normalization(conv2, training=isTrain))

        # 3rd hidden layer
        conv3 = tf.layers.conv2d_transpose(relu2, filter_num[2], [4, 4], strides=(2, 2), padding='same')
        relu3 = relu(tf.layers.batch_normalization(conv3, training=isTrain))

        # 4th hidden layer
        conv4 = tf.layers.conv2d_transpose(relu3, filter_num[3], [4, 4], strides=(2, 2), padding='same')
        ...
        conv6 = tf.layers.conv2d_transpose(relu4, 1, [4, 4], strides=(2, 2), padding='same')
        o = tf.nn.tanh(conv6)
        tf.add_to_collection('G_z', o)
        return o

# D(x)
def discriminator(x, cat_list, conts, isTrain=True, reuse=False):
    with tf.variable_scope('discriminator', reuse=reuse):
        # 1st hidden layer
        # conv0 = tf.layers.conv2d(x, filter_num[4], [4, 4], strides=(2, 2), padding='same')
        # lrelu0 = lrelu(conv0, 0.2)

        conv1 = tf.layers.conv2d(x, filter_num[3], [4, 4], strides=(2, 2), padding='same')
        lrelu1 = lrelu(tf.layers.batch_normalization(conv1, training=isTrain), 0.2)

        # 2nd hidden layer
        conv2 = tf.layers.conv2d(lrelu1, filter_num[2], [4, 4], strides=(2, 2), padding='same')
        lrelu2 = lrelu(tf.layers.batch_normalization(conv2, training=isTrain), 0.2)

        # 3rd hidden layer
        conv3 = tf.layers.conv2d(lrelu2, filter_num[1], [4, 4], strides=(2, 2), padding='same')
        lrelu3 = lrelu(tf.layers.batch_normalization(conv3, training=isTrain), 0.2)

        # 4th hidden layer
        conv4 = tf.layers.conv2d(lrelu3, filter_num[0], [4, 4], strides=(2, 2), padding='same')
        lrelu4 = lrelu(tf.layers.batch_normalization(conv4, training=isTrain), 0.2)

        # output layer for discriminator
        dout = tf.layers.conv2d(lrelu4, 1, [8, 8], strides=(1, 1), padding='valid')
        o = tf.nn.sigmoid(dout)

        # output layer for Q-network
        q_a = tf.reduce_mean(lrelu4, [1,2])
        # q_a = tf.space_to_depth(q1, 2)
      
        ## Here we define the unique layers used for the q-network. The number of outputs depends on the s of 
        ## latent variables we choose to define.
        q_cat_outs = []
        for idx,var in enumerate(cat_list):
            q_out = tf.layers.dense(q_a, var)
            q_outA = tf.nn.softmax(q_out)
            q_cat_outs.append(q_outA)
        
        q_cont_outs = None
        if conts > 0:
            q_cont = tf.layers.dense(q_a, conts)
            q_cont_outs = tf.nn.tanh(q_cont)
        return o, dout, q_cat_outs, q_cont_outs

def show_result(num_epoch, show = False, save = False, path = 'result.png'):
    size_figure_grid = 10
    zs = np.random.uniform(-1.0,1.0,size=[size_figure_grid*size_figure_grid,z_size]).astype(np.float32) #Generate a random z batch
    lcat = np.random.randint(0,cat_num,[size_figure_grid*size_figure_grid,len(categorical_list)]) #Generate random c batch
    lcont = np.random.uniform(-1,1,[size_figure_grid*size_figure_grid,num_continuous]) #
    # fixed_z_ = np.random.uniform(0, 1, (size_figure_grid*size_figure_grid,)+feature_vector_dim)
    test_images = sess.run(G_z, {z_in: zs, latent_cat_in:lcat, latent_cont_in:lcont, isTrain: False})
...
categorical_list = [4] # each entry defines a categorical variable 
cat_num = categorical_list[0]
num_continuous = 2 # number of continous variables
feature_vector_dim = (4, 4, 1)
z_size = np.prod(np.array(feature_vector_dim)) - categorical_list[0] - num_continuous
batch_size = 30
lr = 0.0005
train_epoch = int(1.5e4)
img_dim = (128,128,1)
...
data_path = path + 'data_4sets_1000.hdf5' #needs to be a .mat file
data_var_name = 'img_collection'
img_collection_data = h5py.File(data_path, 'r')
img_collection = np.array(img_collection_data[data_var_name])
del img_collection_data
# variables : input
#These placeholders are used for input into the generator and discriminator, respectively.
z_in = tf.placeholder(tf.float32, shape=[None,z_size], name='z_in') #Random vector
...
latent_cat_in = tf.placeholder(shape=[None,len(categorical_list)],dtype=tf.int32, name='latent_cat_in')
latent_cat_list = tf.split(latent_cat_in,len(categorical_list),1)
latent_cont_in = tf.placeholder(shape=[None,num_continuous],dtype=tf.float32, name='latent_cont_in')
...
z_lat = tf.reshape(z_lat, (-1,)+feature_vector_dim, name='z_lat')
...
# networks : generator
G_z = generator(z_lat, isTrain)

# networks : discriminator
D_real, D_real_logits, _, _ = discriminator(x, categorical_list, num_continuous, isTrain)
D_fake, D_fake_logits, QgCat, QgCont = discriminator(G_z, categorical_list, num_continuous, isTrain, reuse=True)
...
Q_vars = [var for var in T_vars if var.name.startswith('discriminator')]
...
with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
    ...
    Q_optim = tf.train.AdamOptimizer(lr, beta1=0.5, epsilon=1e-7).minimize(Q_loss, var_list=Q_vars)
...
for epoch in range(train_epoch):
    ...
    for D_i in xrange(D_steps):
        ...
        lcat = np.random.randint(0,cat_num,[batch_size,len(categorical_list)]) #Generate random c batch
        lcont = np.random.uniform(-1,1,[batch_size,num_continuous]) #
        # z_ = np.random.uniform(-1, 1, (batch_size,)+feature_vector_dim)

        loss_d_, _, _ = sess.run([D_loss, D_optim, D_clip], {z_in: zs, x: x_, latent_cat_in:lcat, latent_cont_in:lcont, isTrain: True})
        D_losses.append(loss_d_)
        loss_q_, _, _ = sess.run([Q_loss, Q_optim, D_clip], {z_in: zs, x:x_, latent_cat_in:lcat, latent_cont_in:lcont, isTrain: True})
        Q_losses.append(loss_q_)
    for G_i in xrange(G_steps):
        ...
        loss_g_, _ = sess.run([G_loss, G_optim], {z_in: zs, x:x_, latent_cat_in:lcat, latent_cont_in:lcont, style_image:style_image_input, isTrain: True})
        G_losses.append(loss_g_)

    epoch_end_time = time.time()
    per_epoch_ptime = epoch_end_time - epoch_start_time
    print('[%d/%d] - ptime: %.2f loss_d: %.3f, loss_g: %.3f, loss_q: %.3f' % ((epoch + 1), train_epoch, per_epoch_ptime, np.mean(D_losses), np.mean(G_losses), np.mean(Q_losses)))
    ...
    train_hist['Q_losses'].append(np.mean(Q_losses))
